...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        dim_e2h = (self.n_features * self.embed_size, self.hidden_size)
        self.embed_to_hidden_weight = nn.Parameter(torch.empty(dim_e2h))
        ...
        # Initialize weight with the `nn.init.xavier_uniform_` function and bias with `nn.init.uniform_`
        # with default parameters.
        dim_h2l = (self.hidden_size, self.n_classes)
        self.hidden_to_logits_weight = nn.Parameter(torch.empty(dim_h2l))
    ...
    def embedding_lookup(self, w):
        ...
        dim_i = batch_size * self.n_features 
        dim_o = (batch_size, self.n_features * self.embed_size)
        x = torch.index_select(self.embeddings, 0, w.view(dim_i)).view(dim_o)
    ...
    def forward(self, w):
        ...
        x = self.embedding_lookup(w) # get word vector embeddings
        h = relu(x @ self.embed_to_hidden_weight + self.embed_to_hidden_bias) # relu(xw + b)
        h = self.dropout(h)
