...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        ###     Implement a single parsing step

        # Shift: remove first word from buffer, append to stack
        if transition == "S":
            if len(self.buffer) == 0:
                print("Cannot shift with empty buffer.")
            self.stack.append(self.buffer.pop(0))

        # Left Arc: add to list of tuples, remove second most recently added item
        elif transition == "LA":
            if len(self.stack) < 2:
                print("Cannot LEFT-ARC with fewer than two item in stack.")
            self.dependencies.append((self.stack[-1], self.stack[-2]))
            self.stack.pop(-2)
        
        # Right Arc: add to list of tuples, remove most recently added item
        elif transition == "RA":
            if len(self.stack) < 2:
                print("Cannot RIGHT-ARC with fewer than two item in stack.")
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        # use model to predict the next transition for each partial parse in the minibatch
        transitions = model.predict(minibatch)

        # perform a parse step on each partial parse in the minibatch w its predicted transition
        for idx in range(len(minibatch)):
            minibatch[idx].parse_step(transitions[idx])

        # remove completed parses from unfinshed_parses
        for parse_ in minibatch:
            if len(parse_.buffer) == 0 and len(parse_.stack) == 1:
                unfinished_parses.remove(parse_)

    # return dependencies for each (now completed) parse in partial_parses
    dependencies = [parse_.dependencies for parse_ in partial_parses]
