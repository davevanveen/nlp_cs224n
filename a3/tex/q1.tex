    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \subpart[2]First, Adam uses a trick called {\it momentum} by keeping track of $\bm$, a rolling average of the gradients:
                ...
                {\color{red}
                Using $\bm$ dampens the noise in each individual gradient update. By taking an exponentially-weighted rolling average of past gradient steps, the algorithm converges toward the minima faster.
                }
                \\
            ...
            \subpart[2] Adam extends the idea of {\it momentum} with the trick of {\it adaptive learning rates} by keeping track of  $\bv$, a rolling average of the magnitudes of the gradients:
                ...
                {\color{red}
                The parameters with smaller gradients i.e. $| \nabla_{\btheta_t} J (\btheta_t) |$ will get larger updates. This helps with learning in that if a gradient is steep, we dampen the learning rate to avoid descending too far. If a gradient is shallow, though, there is less likelihood that we will take a harmful step, so we can feel confident with a higher learning rate.  
                }
                \\
            ...
            \subpart[2]
                ...
                \\
                {\color{red}
                $$\mathbb{E}_{p_{\text{drop}}}[\bh_\text{drop}] = h = \mathbb{E}[\gamma \bd \odot \bh]= \gamma (1-p_{\text{drop}}) \odot \bh$$
          ...
          \subpart[2] Why should dropout be applied during training? Why should dropout \textbf{NOT} be applied during evaluation? (Hint: it may help to look at the paper linked above in the write-up.) \newline
            ...
            {\color{red}
            Dropout essentially creates multiple sub-networks that share the same features to train on. In a sense this regularizes against overfitting, helping to eliminate complex variable interactions that may arise by using all weights for ``one'' model. At test time I suppose one could pass the input to each sub-network and average the subsequent output, but this would be computationally expensive; besides, we already received the benefit of dropout in training by mitigating overfitting.
            }
                \\
