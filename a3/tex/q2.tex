...
\begin{parts}
    ...
    \textbf{Note}: replaced ``ROOT'' with ``R'' and ``attended'' w ``atd.'' to mitigate compromising margin space.
    
    \begin{tabular}{ l | l | l | l}
    Stack & Buffer & New dep. & Transition \\ \hline
    [R] & [I, atd., lectures, in, the, NLP, class] &  & Initial Config. \\
    $[$R, I] & [atd., lectures, in, the, NLP, class] &  &  \texttt{SHIFT}  \\
    $[$R, I, atd.] & [lectures, in, the, NLP, class] &  &  \texttt{SHIFT}  \\
    $[$R, atd.] & [lectures, in, the, NLP, class] & atd.$\to$I &  \texttt{LEFT-ARC}  \\
    $[$R, atd., lectures] & [in, the, NLP, class] & & \texttt{SHIFT} \\
    $[$R, atd.] & [in, the, NLP, class] & atd.$\to$lectures & \texttt{RIGHT-ARC} \\
    $[$R, atd., in] & [the, NLP, class] & & \texttt{SHIFT} \\
    $[$R, atd., in, the] & [NLP, class] & & \texttt{SHIFT} \\
    $[$R, atd., in, the, NLP] & [class] & & \texttt{SHIFT} \\
    $[$R, atd., in, the, NLP, class] & & & \texttt{SHIFT} \\
    $[$R, atd., in, the, class] & & class$\to$NLP & \texttt{LEFT-ARC} \\
    $[$R, atd., in, class] & & class$\to$the & \texttt{LEFT-ARC} \\
    $[$R, atd., class] & & class$\to$in & \texttt{LEFT-ARC} \\
    $[$R, atd.] & & atd.$\to$class & \texttt{RIGHT-ARC} \\
    $[$R] & & R$\to$atd. & \texttt{RIGHT-ARC} \\
    \end{tabular}\newline
    ...
    {\color{red}
    $2n$. To complete the parsing, each word must be added to the stack and also removed from the stack as a dependency. Hence this requires two steps for each word.
    }
   ...
   In \texttt{parser\_model.py} you will find skeleton code to implement this simple neural network using PyTorch. Complete the \texttt{\_\_init\_\_}, \texttt{embedding\_lookup} and \texttt{forward} functions to implement the model. Then complete the \texttt{train\_for\_epoch} and \texttt{train} functions within the \texttt{run.py} file.
    ...
    {\color{red}    
    Best UAS on dev set: 88.54. UAS on test set: 88.75.
    }
...
\begin{tabular}{ l | l | l | l}
    Sentence & Error Type & Incorrect dependency & Correct dependency \\ \hline
    i & Verb phrase attachment error & acquisition $\rightarrow$ citing & blocked $\rightarrow$ citing   \\
    ii & Modifier attachment error & left $\rightarrow$ early & afternoon $\rightarrow$ early   \\
    iii & Prepositional phrase attachment error & declined $\rightarrow$ decision & reasons $\rightarrow$ decision   \\
    iv & Coordination attachment error & affects $\rightarrow$ one & plants $\rightarrow$ one   \\
    \end{tabular}\newline
...
{\color{red}
    Supplying a part-of-speech tag informs the model with very useful information for learning dependencies among different parts of speech. For instance, it may learn how a verb phrase is attached much easier by having high confidence in what is a ``verb.'' Without these tags, I suspect the model would have a much more difficult time parsing correctly, or at least require much more data to achieve the same performance. 
    }
