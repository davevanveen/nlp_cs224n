...
def train(parser, train_data, dev_data, output_path, batch_size=1024, n_epochs=10, lr=0.0005):
    ...
    # Construct Adam Optimizer in variable `optimizer`
    optimizer = optim.Adam(parser.model.parameters())
    
    # Construct the Cross Entropy Loss Function in variable `loss_func` with `mean`
    # reduction (default)
...
def train_for_epoch(parser, train_data, dev_data, optimizer, loss_func, batch_size):
    ...
    with tqdm(total=(n_minibatches)) as prog:
        for i, (train_x, train_y) in enumerate(minibatches(train_data, batch_size)):
            ...
            # reference: https://pytorch.org/docs/stable/optim.html#optimizer-step
            optimizer.step()
