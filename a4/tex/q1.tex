...
\end{lstlisting}
    ...
    \ifans{The purpose of the masks is, when applying the softmax function, $e_t$ values will be set to $- \infty$ if they correspond to a `pad' token in the input. Hence they will be given no weight and no attention. It's necessary to use this mask, because otherwise we would give attention to pad tokens.}
\end{parts}
...
\end{lstlisting}
    ...
    \ifans{Dot product has the disadvantage of requiring that encoder and decoder have the same dimensionality. It has the advantage of introducing fewer parameters than multiplicative attention, although there are methods to reduce number of parameters in multiplicative attention such as reduced-rank.
    
    Additive attention has the disadvantage of often being slower and less space efficient than multiplicative attention, but it has the advantage of performing better on larger dimensions.}
\end{parts}
