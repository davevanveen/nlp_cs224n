...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.embed_size = embed_size # additional variable to access in class methods
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # source_padded: [src_len = max source sentence length, b = batch_size]
        src_len, b = source_padded.shape
        x_dims = [src_len, b, self.embed_size]
        X = self.model_embeddings.source(source_padded.reshape(-1)).reshape(x_dims)
        ...
        X = torch.permute(X, (1,2,0)) # want [b, e, src_len] --> .permute(x, (1, 2, 0))
        X = self.post_embed_cnn(X)
        X = torch.permute(X, (2,0,1)) # want [src_len, b, e] --> .permute(x, (2, 0, 1))
        ...
        last_hidden_stacked = torch.cat((last_hidden[0], last_hidden[1]), 1)
        init_decoder_hidden = self.h_projection(last_hidden_stacked)
        ...
        last_cell_stacked = torch.cat((last_cell[0], last_cell[1]), 1)
        init_decoder_cell = self.c_projection(last_cell_stacked)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        y_dims = [tgt_len, batch_size, self.embed_size]
        Y = self.model_embeddings.target(target_padded.reshape(-1)).reshape(y_dims)
