...
\begin{answer}
\begin{align}
\frac{\partial J(\bm v_c, o, \bm U)}{\partial v_c}
&= - u_o + \frac{1}{\sum_{w} \exp(\bm u_{w}^\top \bm v_c)} * \sum_{w} u_w^\top \exp(\bm u_{w}^\top \bm v_c) \\
&= - u_o + \sum_{w} u_w^\top \frac{\exp(\bm u_{w}^\top \bm v_c)}{\sum_{w} \exp(\bm u_{w}^\top \bm v_c)} \\
&= - u_o + \sum_{w} u_w^\top \hat{y}_w \\
&= - u_o + U \hat{y} \\
&= - Uy + U \hat{y} &~ \mbox{because $y$ one-hot encoded} \\
&= U(\hat{y} - y)
\end{align}

(ii) The gradient is zero when $\hat{y} = y$, i.e. when the prediction matches the true distribution.

(iii) Say we want to perform a gradient step with step size $\eta$ by computing
$$v_c - \eta \frac{\partial J}{\partial v_c} = v_c - \eta U(\hat{y} - y) = v_c - \eta (\sum_w u_w^\top \hat{y}_w - u_o)$$
If $u_o < \sum_w u_w^\top \hat{y}_w$, then the gradient is positive, and we need to place more probability mass on that word vector. The converse is also true.

(iv) Normalizing via the operation $\mathbf{u}/||\mathbf{u}||_2$ is akin to projecting the vector $\mathbf{u}$ onto the L2 (Euclidean) ball. This is often very useful to keep values within a certain range of 0 to 1. However, we lose information on the magnitude of $\mathbf{u}$. Hence if we had an instance where all that differed between two vectors was their magnitude i.e. $\mathbf{u}_x = \alpha\mathbf{u}_y$ for some scalar $\alpha$, then L2 normalization would project these vectors to be the same. 
\end{answer}
\end{shaded}
...
\begin{align}
\frac{\partial\bm J_{\text{neg-sample}}(\bm v_c, o, \bm U)}{\partial u_o} 
&= -\frac{\partial}{\partial u_o} \log(\sigma(\bm u_o^\top \bm v_c)) \\
&= - (1 - \sigma(\bm u_o^\top \bm v_c)) v_c   &~ \mbox{following lines in $\frac{\partial J}{\partial v_c}$} \\
&= - 1 + \sigma(\bm u_o^\top \bm v_c)) v_c
\end{align}

\begin{align}
\frac{\partial\bm J_{\text{neg-sample}}(\bm v_c, o, \bm U)}{\partial u_{w_s}} 
&= -\frac{\partial}{\partial u_{w_s}} \sum_{s=1}^K \log(\sigma(-\bm u_{w_s}^\top \bm v_c))  \\
&= \sum_{s=1}^K (1 - \sigma(-\bm u_{w_s}^\top \bm v_c)) u_{w_s}  &~ \mbox{following lines in $\frac{\partial J}{\partial v_c}$} 
\end{align}

(ii) \\
Intuitively we can re-use the first and second terms in $\frac{\partial\bm J}{\partial v_c}$ for computing $\frac{\partial\bm J}{\partial u_o} $ and $\frac{\partial\bm J}{\partial u_{w_s}}$, respectively.
Hence given the definitions above, this can be written as $\bm{1} - \sigma(\bm{U}_{o, \{w_1, \dots, w_K\}}^\top v_c)$ \\

(iii) \\
This avoids the flaw in naive softmax, which requires many vector products to be re-calculated since the gradient involves a sum over all words. 
\end{answer}
\end{shaded}
...
\frac{\partial\bm J_{\text{neg-sample}}(\bm v_c, o, \bm U)}{\partial u_{w_s}} 
&= -\frac{\partial}{\partial u_{w_s}} \sum_{s=1}^K \log(\sigma(-\bm u_{w_s}^\top \bm v_c))  \\
&= -\frac{\partial}{\partial u_{w_s}} \sum_{\substack{x=1..K \\ w_x = w_s}} \log(\sigma(-\bm u_{w_x}^\top \bm v_c)) -\frac{\partial}{\partial u_{w_s}} \sum_{\substack{x=1..K \\ w_x \neq w_s}} \log(\sigma(-\bm u_{w_x}^\top \bm v_c)) \\
&= -\frac{\partial}{\partial u_{w_s}} \sum_{\substack{x=1..K \\ w_x = w_s}} \log(\sigma(-\bm u_{w_x}^\top \bm v_c)) \\
&= \sum_{\substack{x=1..K \\ w_x = w_s}} (1 - \sigma(-\bm u_{w_x}^\top \bm v_c)) v_c \\
&= \sum_{x=1..K} \mathbbm{1}_{\{ u_{w_x} = u_{w_s}  \}} (1 - \sigma(-\bm u_{w_x}^\top \bm v_c)) v_c
\end{align}
\end{answer}
\end{shaded}
